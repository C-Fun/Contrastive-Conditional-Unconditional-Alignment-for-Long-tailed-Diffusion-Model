...
def brs(index, imb_data, imb_targets, cls_num, brsf=1.0):
    prob = [brsf ** (cls_idx / (cls_num - 1)) for cls_idx in range(cls_num)]
...
class ImbalanceCIFAR10(datasets.CIFAR10):
    ...
    def __init__(self, root, imb_type='exp', imb_factor=0.01, rand_number=0, brs=False, brs_f=1.0, train=True,
        ...
        self.brs, self.brs_f = brs, brs_f

    def __getitem__(self, index):
        if self.brs:
            index = brs(index, self.data, self.targets, self.cls_num, brsf=self.brs_f)
    ...
    # def gen_balanced_resample_data(self):
    #     targets_np = np.array(self.targets, dtype=np.int64)
    #     classes = np.unique(targets_np)
    #
    #     ydist = list(self.num_per_cls_dict.values())
    #     img_num_per_cls = [max(ydist)] * len(ydist)
    #
    #     brs_indice = []
    #     for the_class, the_img_num in zip(classes, img_num_per_cls):
    #         idx = np.where(targets_np == the_class)[0]
    #         selec_idx = np.random.choice(idx, the_img_num, replace=True)
    #         brs_indice.append(selec_idx)
    #     brs_indice = np.hstack(brs_indice)
    #
    #     # print(self.data.shape, targets_np.shape)
    #     self.data = self.data[brs_indice, ...]
    #     self.targets = targets_np[brs_indice, ...]
...
class ImbalanceImageNet(ImageNet):
    def __init__(self, root, split="train", imb_type="exp", imb_factor=1.0, rand_number=0, brs=False, brs_f=1.0,
    ...
    def __getitem__(self, index):
        if self.brs:
            index = brs(index, self.samples, self.targets, self.cls_num, brsf=self.brs_f)
...
if __name__ == "__main__":
    ...
    # dataset = ImbalanceCIFAR10(root='.cache/data/',
    ...
    dataset = ImbalanceImageNet(root='/mnt/data2/imagenet2012/',
                                split="train", imb_type='exp', imb_factor=0.01, rand_number=0,
                                brs=True, brs_f=0.1,
                                transform=tran_transform,
                                )

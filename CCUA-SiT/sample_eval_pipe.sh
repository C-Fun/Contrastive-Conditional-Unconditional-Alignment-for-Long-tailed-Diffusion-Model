...
torchrun --nnodes=1 --nproc_per_node=$N_GPUS --master-port $MASTER_PORT ./SiT/sample_ddp.py ODE \
         --image-size 256 --num-sampling-steps 50 --cfg-scale $CFG_SCALE --per-proc-batch-size $SAMPLE_BS \
         --model SiT-S/2 --num-fid-samples 50000 --ckpt "$LATEST_CKPT" --sample-dir "samples/$PROJECT/"

# The .npz file is named after the folder created by sample_ddp.py
CKPT_NAME=$(basename "$LATEST_CKPT" .pt)
SAMPLE_NPZ="samples/$PROJECT/${MODEL_NAME}-${CKPT_NAME}-cfg-${CFG_SCALE}-${SAMPLE_BS}-ODE-50-dopri5.npz"
EVAL_TXT="samples/$PROJECT/${MODEL_NAME}-${CKPT_NAME}-cfg-${CFG_SCALE}-${SAMPLE_BS}-ODE-50-dopri5.txt"
